#!/usr/bin/env bash
# 03_experiment.sh — PHASE 3: run the delivery experiment and collect data.
# For every lead_time it does: clean -> build+start (delivery armed) -> register ->
# wait until past the last delivery slot -> aggregate Singapore per-slot vote ->
