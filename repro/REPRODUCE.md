...
The public IP addresses in this table are deployment-specific endpoints used for the author's
experiment. They are not required for reproduction. Evaluators must substitute their own hosts,
restrict access to their own security groups, and must not use the author's endpoints or keys.
...
> when built. Nothing besides ssh/scp is needed locally.
...
this is the only place you change machine-specific values. The endpoint values used in the author's
experiment are deployment-specific; evaluators must replace them with their own three hosts and must
not use the author's endpoints or keys.
...
not select the separate `node-deploy` repository. This reproduction requires the
`attack-3` branch, which provides `bsc_cluster_multi.sh` and the matching deployment
...
Before running Phase 1, switch the local checkout to `attack-3`:

```bash
git -C node-deploy switch attack-3
git -C node-deploy branch --show-current   # should print: attack-3
```

The archive intentionally includes the `node-deploy` Git metadata and the
`attack-3` ref, so a fresh Phase 1 unpack can use this branch without network access
...
ssh -i pem/<region-key>.pem ubuntu@<region-ip> \
  'cd ~/bsc-new-attack-experiment && rm -rf node-deploy && unzip -q -o node-deploy.zip && git -C node-deploy switch attack-3 && git -C node-deploy branch --show-current'
```
